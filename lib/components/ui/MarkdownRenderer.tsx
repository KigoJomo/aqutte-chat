    ...
    <div className={`${className} max-w-full prose dark:prose-invert *:!transition-all *:!duration-300`}>
      <ReactMarkdown
        ...
        components={{
          code: ({
            ...
            return !isInlineCode ? (
              <div className="code-block-wrapper border border-foreground-light/10 rounded-xl my-0 overflow-hidden">
                <div className="flex justify-between items-center px-4 py-1 bg-[#0f0f0f] border-b border-foreground-light/10">
                  <div className="flex items-center gap-2">
                    <CodeXml size={16} className="stroke-foreground fill-accent" />
          ...
          th: ({ ...rest }) => (
            <th
              className="px-2 py-2 bg-background-dark font-semibold text-left"
              {...rest}
            />
          ),
          td: ({ ...rest }) => (
            <td
              className="bg-background-dark/50 hover:bg-background-light px-2 py-3 font-light text-sm transition-all duration-300"
              {...rest}
            />
          ),
        }}>
